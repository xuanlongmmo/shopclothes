...
@section('content')
    <div style="margin-left: 20px;margin-bottom: 20px">
        ...
        <div class="hang">
            ...
            <div class="cot">
                <span class="up">Ghi chú</span>
                <div class="ogrey">
                    <span style="font-size: 14px">{{ $data->note }}</span>
                </div>
            </div> 
        </div>  
        <table class="ok" id="ok">
            <thead>
        ...
        @if ($data->id_status == 1)
            <div style="display: flex;width: 30%;margin: auto;">
                <a href="{{ route('admin.updateorder', ['id'=>$data->id,'status'=>2]) }}"><button style="background-color: green;border: green solid 1px"><span style="color: black">Xác nhận đơn</span></button></a>
                <a href="{{ route('admin.updateorder', ['id'=>$data->id,'status'=>4]) }}"><button style="margin-left: 10px;background-color: red;border: red solid 1px"><span style="color: black">Hủy đơn</span></button></a>
            </div>
        @elseif ($data->id_status == 2)
            <div style="display: flex;width: 30%;margin: auto;">
                <a href="{{ route('admin.updateorder', ['id'=>$data->id,'status'=>3]) }}"><button style="background-color: green;border: green solid 1px"><span style="color: black">Giao hàng thành công</span></button></a>
                <a href="{{ route('admin.updateorder', ['id'=>$data->id,'status'=>4]) }}"><button style="margin-left: 10px;background-color: red;border: red solid 1px"><span style="color: black">Hủy đơn</span></button></a>
            </div>
        @endif
    </div>
